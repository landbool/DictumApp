...
class DictumBridge(ctk.CTk):
    ...
    def show_main_app_smoothly(self):
        ...
        # Запускаем фоновую проверку обновлений при успешном показе окна
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if latest_version > CURRENT_VERSION:
                        ...
                        if download_url:
                            # Безопасно вызываем окно предложения в главном потоке GUI
                            self.after(0, lambda: self.show_update_dialog(latest_version, download_url))
            except Exception as e:
                print(f"[Update Checker] Не удалось проверить обновления: {e}")
    ...
    def show_update_dialog(self, new_version, url):
        answer = messagebox.askyesno(
            "Доступно обновление!", 
            f"Вышла новая версия Dictum v{new_version}.\n\nХотите скачать и установить её прямо внутри приложения?"
        )
        if answer:
            # Запускаем красивый внутренний загрузчик
            self.start_in_app_download(url)

    def start_in_app_download(self, url):
        # Создаем маленькое стильное окошко поверх программы
        progress_win = ctk.CTkToplevel(self)
        progress_win.title("Обновление системы")
        progress_win.geometry("340x140")
        ...
        lbl_percent = ctk.CTkLabel(progress_win, text="0%", font=("Consolas", 11), text_color="gray50")
        ...
        def download_worker():
            try:
                ...
                # Потоковое скачивание файла частями (stream=True)
    ...
    def run_installer_and_exit(self, setup_path, win_to_close):
        ...
        # Открываем скачанный установщик силами самой Windows
        os.startfile(setup_path)
        
        # КРИТИЧЕСКИ ВАЖНО: Мгновенно убиваем текущий запущенный Dictum.exe.
        # Если этого не сделать, установщик выдаст ошибку "Файл занят другим процессом"
        # и не сможет обновить файлы в AppData/Local!
        self.quit()
        sys.exit()
